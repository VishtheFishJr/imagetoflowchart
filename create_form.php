...
error_log(
    'GOOGLE TOKEN INFO: ' .
    print_r($client->getAccessToken(), true)
);
...
if ($httpCode < 200 || $httpCode >= 300) {
    ...
    if (
        ...
        $errMsg =
            'Google Forms permission is missing. Please reconnect your Google account.';
    }
...
if ($formDescription !== '') {
    ...
    if (
        ...
        $descErrMsg =
            ...
            'Google Forms rejected the form description.';

        if (
            $descriptionHttpCode === 403 &&
            (
                stripos($descErrMsg, 'scope') !== false ||
                stripos($descErrMsg, 'permission') !== false ||
                stripos($descErrMsg, 'insufficient') !== false
            )
        ) {
            $descErrMsg =
                'Google Forms permission is missing. Please reconnect your Google account.';
...
if (count($requests) > 0) {
    ...
    if (
        ...
        if (
            $batchHttpCode === 403 &&
            (
                stripos($batchErrMsg, 'scope') !== false ||
                stripos($batchErrMsg, 'permission') !== false ||
                stripos($batchErrMsg, 'insufficient') !== false
            )
        ) {
            $batchErrMsg =
                'Google Forms permission is missing. Please reconnect your Google account.';
